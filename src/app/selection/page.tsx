...
import { GameInitializationUseCase, PokemonSet } from '@/app/usecases/GameInitializationUseCase';
...
export default function StarterSelectionPage() {
  ...
  useEffect(() => {
    async function generateSets() {
      try {
        const gateway = new PokemonAPIGateway();
        const availablePokemons: Pokemon[] = [];
        const pokemonIds = Array.from({ length: 300 }, (_, i) => i + 1); // Utilise les 500 premiers Pokémon de Tyradex
        ...
        const useCase = new GameInitializationUseCase();
  ...
  const handleSelectSet = async () => {
    if (sets[selectedSetIndex]) {
      ...
      // Générer les trainers de la ligue une seule fois ici
      const LeagueUseCase = (await import('@/app/usecases/LeagueUseCase')).LeagueUseCase;
      ...
      // Redirection vers la page de la ligue
      router.push('/league');
    }
  };
