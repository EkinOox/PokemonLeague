export class TypeEffectivenessService {
  private typeChart: { [key: string]: { [key: string]: number } } = {
    ...
    bug: { grass: 2, fighting: 0.5, poison: 0.5, flying: 0.5, psychic: 2, ghost: 0.5, dark: 2, steel: 0.5, fairy: 0.5 },
  ...
  getMultiplier(attackerType: string, defenderType: string): number {
    ...
    if (defenderType in attackerChart) {
      return attackerChart[defenderType];
    }
    return 1;
  }
}